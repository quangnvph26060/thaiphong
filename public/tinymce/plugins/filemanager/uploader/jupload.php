...
class JUpload {
    ...
    private function tobytes($val) {
        $val = trim($val);
        $last = strtolower($val[strlen($val) - 1]); // Use square brackets instead of curly braces

        switch ($last) {
            case 'g':
                $val = (float)$val * 1024 * 1024 * 1024; // Convert GB to bytes
                break;
            case 'm':
                $val = (float)$val * 1024 * 1024; // Convert MB to bytes
                break;
            case 'k':
                $val = (float)$val * 1024; // Convert KB to bytes
                break;
            default:
                $val = (float)$val; // Assume bytes if no suffix is provided
                break;
        }
